...
class UDGGeneratorSeed:
    ...
    Attributes:
        ...
        min_dist: Minimum distance between nodes in the graph.
    ...
    def __init__(
            ...
            min_dist: float,
            radius: float,
            coverage_bound: list[float],
            avg_deg_bound: list[float],
            prob_connected: float,
            sample_size: int = 10,
            graphs: list[LambdaPrecisionUDG] = None
    ) -> None:
        """ Initialises an instance of the class providing the parameters needed to define graph and network properties such as number of nodes, distance metrics, coverage parameters, and other bounds. The instance can also include predefined graphs or define parameters for random sampling of graphs.

        Args:
            node_number: The total number of nodes in the graph.
            min_dist: Minimum distance allowed between any two nodes.
            radius: Effective radius for defining neighbourhood relationships.
            coverage_bound: A list containing the lower and upper bounds of the coverage area values.
            avg_deg_bound: A list containing the lower and upper bounds of the average degree of nodes.
            prob_connected: Probability that the graph will be connected.
            sample_size: The number of random samples of graphs to generate. Defaults to 10.
            graphs: A list of pre-defined `LambdaPrecisionUDG` graphs to initialise the instance with. Defaults to None.
        """
    ...
    def probability_connected(self) -> float:
        ...
        """
        self.prob_connected = float(np.mean([nx.is_connected(graph.graph) for graph in self.graphs]))
    ...
    def get_avg_coverage(self) -> float:
        ...
        return float(np.mean([graph.lambda_precision_points.get_density() for graph in self.graphs]))
    ...
    def variance_node_degree_distribution(self, sample_size: int = 0) -> list[float]:
        ...
        This method computes the variance of the degree distribution for a given sample size of the graph's nodes. It utilizes the degree distribution values to derive the variance and returns them as a list of floats. This can be useful to analyse the spread or dispersion of node connectivity in the graph structure.
