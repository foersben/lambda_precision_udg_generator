...
def test_density_from_metadata(logger: logging.Logger) -> None:
    ...
    for _ in range(20):
        generator = LambdaPrecisionUDGGenerator(
            lambda_precision_points_generator, radius=0.083, logger=logger
        )
        ...
        logger.info(
            f"Original density: {original_density}, Reconstructed density: {reconstructed_density}"
        )
...
def test_lambda_precision_udg_generator(logger: logging.Logger) -> None:
    ...
    for _ in range(20):
        generator = LambdaPrecisionUDGGenerator(
            RandomPointsGenerator(point_number=300, min_dist=0.037), radius=0.083, logger=logger
        )
        graph = generator.generate_graph(connected=True)
...
def test_old_generator_seeds(logger: logging.Logger) -> None:
    ...
    for _ in range(10):
        generator = LambdaPrecisionUDGGenerator(
            RandomPointsGenerator(point_number=100, min_dist=0.086932), radius=0.121875
        )
    ...
    for _ in range(10):
        generator = LambdaPrecisionUDGGenerator(
            RandomPointsGenerator(point_number=300, min_dist=0.048588), radius=0.065625
        )
        graph = generator.generate_graph()
...
def test_connecting_graphs(logger: logging.Logger) -> None:
    ...
    generator = LambdaPrecisionUDGGenerator(
        RandomPointsGenerator(point_number=300, min_dist=0.037), radius=0.0649
    )
    graph = generator.generate_graph()

    graph_depiction.draw_graph_with_segmented_nodes(
        graph, mean_types=5, max_bandwidth=100, save_path="../test_output/test.png"
    )
    ...
    graph_depiction.draw_graph_with_segmented_nodes(
        graph, mean_types=5, max_bandwidth=100, save_path="../test_output/test1.png"
    )
    ...
    graph_depiction.draw_graph_with_segmented_nodes(
        graph, mean_types=5, max_bandwidth=100, save_path="../test_output/test2.png"
    )
