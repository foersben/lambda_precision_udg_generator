...
class GeneratorSeed:
    ...
    def generate_graphs(
        self, sample_size: int, new: bool = True, connected: bool = False, bounds: bool = False
    ) -> None:
        ...
        generator = LambdaPrecisionUDGGenerator(
            RandomPointsGenerator(self.node_number, self.min_distance), self.radius
        )

        while len(self.graphs) < sample_size:
            self.logger.info("Generate graphs...")
            graphs = generator.generate_graphs_parallel(
                max(sample_size - len(self.graphs), 10), connected=connected
            )
            self.graphs.extend(graphs)
            self.logger.debug(
                f"Graph: Node Number: {self.node_number}, "
                f"Lambda: {self.min_distance}, Radius: {self.radius}, "
                f"Avg Degree: {self.avg_deg_bound!s}"
            ...
            if bounds:
                self.graphs = list(
                    filter(
                        lambda graph: self.avg_deg_bound[0]
                        <= graph.average_degree()
                        <= self.avg_deg_bound[1]
                        and self.coverage_bound[0]
                        <= graph.get_lambda_precision_points().get_density()
    ...
    def copy(self) -> "GeneratorSeed":
        ...
        return GeneratorSeed(
            ...
            self.min_distance,
            self.radius,
            deepcopy(self.coverage_bound),
            deepcopy(self.avg_deg_bound),
            self.probability_connected,
            self.sample_size,
        )
    ...
    def probability_connected(self) -> float:
        ...
        self.probability_connected = float(
            np.mean([nx.is_connected(graph) for graph in self.graphs])
        )
    ...
    def get_avg_coverage(self) -> float:
        ...
        return float(
            np.mean([graph.get_lambda_precision_points().get_density() for graph in self.graphs])
        )
    ...
    def local_clustering(self, sample_size: int = 0, new: bool = False) -> list[dict[int, float]]:
        ...
        if not sample_size:
            sample_size = self.sample_size
        self.generate_graphs(sample_size, new)
        return [nx.clustering(graph) for graph in self.graphs[:sample_size]]
    ...
    def __str__(self) -> str:
        ...
        return (
            ...
            f"graphs: {(len(self.graphs), self.graphs)!s}"
        )
    ...
    def to_dict(self) -> dict:
        ...
        metadata["graphs"] = [graph.to_dict() for graph in self.graphs]
        return metadata

    @classmethod
    def from_dict(cls, data: dict) -> "GeneratorSeed":
        ...
        Returns:
            Reconstructed GeneratorSeed object
        """
        from lambdaprecisionudggenerator.graph_generator.graphs.lambda_precision_udg import (
            LambdaPrecisionUDG,
        )
        ...
        # Reconstruct graphs
        graphs = [LambdaPrecisionUDG.from_dict(g_data) for g_data in data.get("graphs", [])]

        # Create instance using from_metadata (without graphs)
        metadata_without_graphs = {k: v for k, v in data.items() if k != "graphs"}
        instance = cls.from_metadata(metadata_without_graphs)
        instance.graphs = graphs

        return instance

    def serialize(self, path: str) -> None:
        """Serializes the current instance of the class to a JSON file. The file is stored at the given path with a unique file name based on the object's ID to avoid collisions.
        ...
        self.logger.info(f"GeneratorSeed.serialize {path}/{id(self)}.json")
        filepath = f"{path}/{id(self)}.json"
        save_json(self.to_dict(), filepath)

    @classmethod
    def deserialize(cls, filepath: str) -> "GeneratorSeed":
        """Deserializes a `GeneratorSeed` object from a JSON file. The method expects the file to be in JSON format. This operation will read the contents of the specified file and return a `GeneratorSeed` object that was previously serialized.

        Args:
            filepath: The file path where the serialized `GeneratorSeed` object is stored.

        Returns:
            A `GeneratorSeed` object deserialized from the specified file.
        """
        from lambdaprecisionudggenerator.utils.json_utils import load_json

        logging.getLogger(__name__).info(f"GeneratorSeed.deserialize {filepath}")
