...
def test_lambda_precision_points_comprehensive():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if len(loaded.points) != len(original.points):
            ...
            for i, (lp, op) in enumerate(zip(loaded.points, original.points)):
                if list(lp) != list(op):
                    errors.append(f"Point {i} mismatch: {lp} != {op}")
        
        # Check min_distance
        if loaded.min_distance != original.min_distance:
            errors.append(f"min_distance mismatch: {loaded.min_distance} != {original.min_distance}")
...
def test_lambda_precision_udg_comprehensive():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if loaded.number_of_nodes() != original.number_of_nodes():
            errors.append(f"Node count mismatch: {loaded.number_of_nodes()} != {original.number_of_nodes()}")
        
        if loaded.number_of_edges() != original.number_of_edges():
            errors.append(f"Edge count mismatch: {loaded.number_of_edges()} != {original.number_of_edges()}")
        
        # Check nodes exist
        if set(loaded.nodes()) != set(original.nodes()):
            errors.append(f"Node set mismatch")
        
        # Check edges exist
        if set(loaded.edges()) != set(original.edges()):
            errors.append(f"Edge set mismatch")
...
def test_generator_seed_comprehensive():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        attrs_to_check = [
            'node_number', 'min_distance', 'radius',
            'probability_connected', 'sample_size'
        ]
        ...
        if list(loaded.coverage_bound) != list(seed.coverage_bound):
            errors.append(f"coverage_bound mismatch: {loaded.coverage_bound} != {seed.coverage_bound}")
        ...
        if len(seed.graphs) > 0:
            if len(loaded.graphs) != len(seed.graphs):
                ...
                for i, (lg, og) in enumerate(zip(loaded.graphs, seed.graphs)):
...
def test_generator_seed_db():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if len(loaded_db.seeds) != len(original_db.seeds):
            errors.append(f"Seed count mismatch: {len(loaded_db.seeds)} != {len(original_db.seeds)}")
        else:
            ...
            for i, (ls, os) in enumerate(zip(loaded_sorted, original_sorted)):
                if ls.node_number != os.node_number:
                    errors.append(f"Seed {i} node_number mismatch: {ls.node_number} != {os.node_number}")
                if ls.min_distance != os.min_distance:
                    errors.append(f"Seed {i} min_distance mismatch: {ls.min_distance} != {os.min_distance}")
...
def test_base_result_comprehensive():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        scalar_attrs = [
            'partition_size', 'opt_type', 'objective', 'wallclock_time',
            'aborted', 'graph_id', 'lbound', 'ubound', 'mipgap'
        ]
