...
def _assign_bandwidth_usage(
    ax: plt.Axes, graph: LambdaPrecisionUDG, max_bandwidth: int = 100
) -> None:
...
def _get_edge_color(bandwidth_usage: int, max_bandwidth: int = 100) -> tuple[float, float, float]:
    ...
    return (
        plt.cm.plasma(bandwidth_usage / max_bandwidth)
        if bandwidth_usage <= max_bandwidth
        else (1, 0, 0, 1)
    )
...
def draw_graph_with_segmented_nodes(
    graph: nx.Graph, mean_types: int, max_bandwidth: int = 100, save_path: str | None = None
) -> None:
