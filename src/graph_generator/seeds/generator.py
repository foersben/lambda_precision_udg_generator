...
class SeedGenerator:
    ...
    def __init__(self, sample_size: int = 10, logger: logging.Logger = None) -> None:
    ...
    def _determine_coverage(
        ...
        while True:
            point_sets = RandomPointsGenerator(point_number=node_number,
                                               min_dist=min_dist["result"]).generate_points_parallel(self.sample_size)
    ...
    def generate_seeds(
        ...
        for node_number in node_numbers:
            min_dist = self._determine_coverage(
                min_dist_config.copy(),
                coverage_bound,
                node_number,
                padding
            )

            point_sets = RandomPointsGenerator(node_number, min_dist).generate_points_parallel(self.sample_size)

            for avg_deg in avg_degs:
                radius, graphs = self._determine_avg_deg(point_sets, avg_deg)

                seeds.append(GeneratorSeed(
                    node_number=node_number,
                    min_distance=min_dist,
                    radius=radius,
                    coverage_bound=coverage_bound,
                    avg_deg_bound=(avg_deg, avg_deg + 0.25),
                    probability_connected=sum(nx.is_connected(graph) for graph in graphs) / float(len(graphs)),
