...
try:
    ...
    logging.getLogger(__name__).warning(
        "tikzplotlib not available or incompatible with current matplotlib version."
    )
...
class GeneratorSeedDB:
    ...
    def __init__(self, *seeds: GeneratorSeed, logger: logging.Logger | None = None) -> None:
    ...
    def _generate_plot(self, config: PlotConfig) -> None:
        ...
        for (node_number, deg), seeds_in_group in groups.items():
            ...
            ax.plot(x, y, label=config.legend_fmt.format(node_number, deg), lw=2)
    ...
    def _generate_latex_table(
        self, columns: list[tuple[str, Callable[[GeneratorSeed], any]]], filepath: str | None = None
    ) -> str:
        ...
        latex_table = tabulate([headers, *table_data], tablefmt="latex_raw", headers="firstrow")
    ...
    def latex_table(self, filepath: str | None = None) -> str:
        columns = [
            ...
            (
                ...
                lambda seed: np.mean(
                    [graph.get_lambda_precision_points().get_density() for graph in seed.graphs]
                ),
            ),
            (
        ...
        data = sorted(
            [
                [
                    ...
                    float(np.mean([graph.average_degree() for graph in seed.graphs])),
                    seed.probability_connected,
                ]
                for seed in self.seeds
            ],
    ...
    def deserialize(cls, path: str) -> "GeneratorSeedDB":
        ...
        directory, locates all files with a `.json` extension, and attempts to deserialise these files. Each deserialised object is then used as an argument to instantiate a new instance of the class.

        This method ensures to verify the existence of the provided path before proceeding
        with deserialization.

        Args:
            path: Path to the directory containing the serialised `.json` files. Must be a valid existing directory.
        ...
        # Deserialize all JSON files
        return cls(*[GeneratorSeed.deserialize(os.path.join(path, j)) for j in json_files])
